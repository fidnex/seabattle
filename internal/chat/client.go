...
func NewClient(room *Room, conn *websocket.Conn, userID string) *Client {
	return &Client{Room: room, conn: conn, send: make(chan []byte, 256), userID: userID}
}
...
func (c *Client) readPump() {
	...
	for {
		...
		var shot domain.Shoot
		err = json.Unmarshal(message, &shot)
		if err == nil {
			shot.UserID = c.userID
			c.Room.shot <- &shot
		}
	}
}
